...
function after_checkout_validation_168( $posted ) {
	if (WC()->session->get('chosen_payment_method') === 'mollie_wc_gateway_bancontact') {
		// nothing
	}
	else {
...
function custom_validate_paypal_fields() {
	...
	if (is_wc_endpoint_url('order-received')) return; // Skip validation on order received page
    if ($chosen_payment_method === 'mollie_wc_gateway_bancontact') {
        ...
        if (empty($first_name) || empty($last_name)) {
            wc_add_notice(__('Please enter both first name and last name for Mollie Bank Transfer payments.', 'woocommerce'), 'error');
        }
    }
}
...
function display_paypal_fields_error_message() {
    if (is_checkout()) {
        $chosen_payment_method = WC()->session->get('chosen_payment_method');
        if ($chosen_payment_method === 'mollie_wc_gateway_bancontact') {
            wc_print_notice(__('Please enter both first name and last name for Mollie Bank Transfer payments.', 'woocommerce'), 'error');
        }
    }
}


add_filter('woocommerce_checkout_fields', 'remove_fields_for_paypal');

function remove_fields_for_paypal($fields) {
    // Check if the selected payment method is PayPal
    if (WC()->session->get('chosen_payment_method') === 'mollie_wc_gateway_bancontact') {
        // List of field keys to remove for PayPal payment
...
add_action('woocommerce_review_order_before_payment', 'remove_fields_for_paypal_filter');

function remove_fields_for_paypal_filter() {
    // Check if the selected payment method is PayPal
    if (WC()->session->get('chosen_payment_method') === 'mollie_wc_gateway_bancontact') {
        // Replace 'billing_company' with the field name you want to remove
       add_filter('woocommerce_checkout_fields', 'remove_fields_for_paypal');
    }
}

// Function to return updated billing fields HTML
function get_updated_billing_fields_html($payment_method) {
    ob_start();
    
    // Modify the billing fields based on the payment method (similar to your previous code)
    $fields = apply_filters('woocommerce_checkout_fields', WC()->checkout->get_checkout_fields('billing'), $payment_method);
    
    // Output the updated fields HTML
    woocommerce_form_field('billing_email', $fields['billing']['billing_email']);
...
// Hook to handle the AJAX request and return updated billing fields HTML
add_action('wp_ajax_refresh_billing_section', 'refresh_billing_section');
add_action('wp_ajax_nopriv_refresh_billing_section', 'refresh_billing_section');

function refresh_billing_section() {
    if (isset($_POST['payment_method'])) {
        $payment_method = sanitize_text_field($_POST['payment_method']);
        $updated_billing_html = get_updated_billing_fields_html($payment_method);
        echo $updated_billing_html;
    }
    die();
}
