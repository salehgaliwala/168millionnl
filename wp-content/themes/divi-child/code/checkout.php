...
add_action('woocommerce_admin_order_data_after_shipping_address', 'edit_custom_field_on_order_edit', 10, 1);

function edit_custom_field_on_order_edit($order) {
    $custom_field = get_post_meta($order->get_id(), '_projectnummer_field', true);
    echo '<p class="form-field custom-field-wrap">
        <label for="custom_field">Project Number:</label>
        <input type="text" class="short" name="_projectnummer_field" value="' . esc_attr($custom_field) . '" />
    </p>';
}

add_action('woocommerce_process_shop_order_meta', 'save_custom_field_on_order_edit', 10, 2);

function save_custom_field_on_order_edit($order_id, $post) {
    if (isset($_POST['_projectnummer_field'])) {
        $custom_value = sanitize_text_field($_POST['_projectnummer_field']);
        update_post_meta($order_id, '_projectnummer_field', $custom_value);
    }
}